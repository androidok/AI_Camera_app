...
void main() {
    ...
    // 根据边缘强度调整：强边缘可以承受更多锐化
    float edgeFactor = smoothstep(0.05, 0.2, edgeStrength);
    // 边缘区域保持强度，非边缘区域降低强度
    adaptiveStrength = mix(adaptiveStrength * 0.3, adaptiveStrength, edgeFactor);

    // 应用自适应锐化
    cur += (mask.r + mask.g + mask.b) * (adaptiveStrength * 4.0 / 3.0);
